...
const ChatInput = () => {
    ...
    const [qSQL, setSQL] = useState<string>()
    ...
    const handleSubmit = () => {
        if (query) {
            axios.post<Interface>("http://127.0.0.1:8000/api/processQuery", {
                queryInput: query
            })
                .then(data => {
    ...
    return (
        <>
            <div className="w-full flex justify-center pb-6 px-4 from-[#000000] via-[#131314] to-transparent absolute bottom-0">
                <div className="w-full max-w-3xl mb-12 scale-90">
                    ...
                    <button type="button" className=' p-4 px-8 bg-neutral-50 text-neutral-800 rounded-xl' onClick={() => handleSubmit()}>Kirim</button>
            ...
            {data &&(
                <ChatArea query={qSQL ? qSQL : "nothing"} data={data}/>
            )}
        </>
    );
};
